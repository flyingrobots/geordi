...
The dead-code guard in `extractNodes.ts` — `dir.name.value !== 'geordi_node'` — is the kind of thing that happens when code evolves. Someone added the KNOWN_GEORDI_DIRS set and forgot to remove the explicit exclusion that predated it. Harmless. But it tells a reader a lie: it implies `geordi_node` needs special handling that it doesn't. Code that lies to its readers is worse than no comment at all. Worth one line to fix.
...
- Runtime type validation for `@geordi_node` directive args — the `as` cast problem
