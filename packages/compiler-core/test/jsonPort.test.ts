...
describe('canonical JSON port', () => {
  ...
  it('rejects non-finite numbers instead of silently changing them', () => {
    expect(() => stringifyCanonicalJson({ value: NaN })).toThrow(JsonNonFiniteNumberError);
    expect(() => stringifyCanonicalJson({ value: Infinity })).toThrow(JsonNonFiniteNumberError);
    expect(() => stringifyCanonicalJson({ value: -Infinity })).toThrow(JsonNonFiniteNumberError);
  });
  ...
  it('preserves array order while canonicalizing nested object keys', () => {
    expect(stringifyCanonicalJson([{ z: 1, a: 2 }, { b: 3 }])).toBe('[{"a":2,"z":1},{"b":3}]');
  });

  it('throws a custom parse error for invalid JSON input', () => {
    expect(() => parseJsonValue('{not json')).toThrow(JsonParseError);
  });
});
