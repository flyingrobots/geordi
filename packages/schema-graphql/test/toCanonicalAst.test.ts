...
describe('toCanonicalAst', () => {
  ...
  it('node id uses deterministicId(node, sceneName, fieldName)', () => {
    const { ast } = compile(`
      type Terminal @geordi_scene(v: "1", width: 800, height: 600) {
        bg: String @geordi_node(kind: Rect, x: 0, y: 0)
      }
    `);
  ...
  it('explicit id overrides deterministicId', () => {
    const { ast } = compile(`
      type Terminal @geordi_scene(v: "1", width: 800, height: 600) {
        bg: String @geordi_node(kind: Rect, x: 0, y: 0, id: "my-custom-id")
      }
    `);
  ...
  it('visible defaults to true when not specified', () => {
    const { ast } = compile(`
      type S @geordi_scene(v: "1", width: 100, height: 100) {
        n: String @geordi_node(kind: Rect, x: 0, y: 0)
      }
    `);
  ...
  it('explicit visible: false is preserved', () => {
    const { ast } = compile(`
      type S @geordi_scene(v: "1", width: 100, height: 100) {
        n: String @geordi_node(kind: Rect, x: 0, y: 0, visible: false)
      }
    `);
  ...
  it('explicit zIndex overrides field order', () => {
    const { ast } = compile(`
      type S @geordi_scene(v: "1", width: 100, height: 100) {
        n: String @geordi_node(kind: Rect, x: 0, y: 0, zIndex: 99)
      }
    `);
  ...
  it('metadata.sourceFormat is graphql-sdl', () => {
    const { ast } = compile(`
      type S @geordi_scene(v: "1", width: 100, height: 100) { _: String }
    `);
  ...
  it('scene name uses name arg when specified', () => {
    const { ast } = compile(`
      type Terminal @geordi_scene(v: "1", width: 800, height: 600, name: "My Terminal") { _: String }
    `);
